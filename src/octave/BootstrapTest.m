...
function results = BootstrapTest(true_model, data, utility, lambda=0.5, n_iter=10000, period = 100, alpha = 0.001, n_samples = 16)
  X = true_model.X;
  Y = true_model.Y;
  Z = true_model.Z;
  A = rows(utility);
  ...
  for policy_index = 1:max_policies
	...
	for time = 1:horizon
	  if (--period_left <= 0 || time == horizon)
		...
		%% get the policy
		if (policy_index == 1) 
		  policy = GetBayesPolicyBootstrap(belief, utility, lambda, alpha, n_iter, policy);
		else					  
