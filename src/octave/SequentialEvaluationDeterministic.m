...
for k=1:K
  ...
  for n_policies = 1:max_policies
	printf("\tPolicies: %d\n", n_policies); fflush(stdout);
	belief = prior_belief;
	for time = 1:horizon
	  policy = SequentialFairnessDeterministic(belief, model, model_delta, utility, lambda(k), n_data, n_samples, n_policies, 0, planning_horizon);
	  PU(k, n_policies) += Utility(policy, model{1}, utility);
	  PF(k, n_policies) += Fairness(policy, model{1}, model_delta{1});
	  PV(k, n_policies) += (1 - lambda(k)) * PU(k, n_policies) - lambda(k) * F(k, n_policies);
	  if (time < horizon)
		belief = GeneratePosteriorFromPolicy(true_belief, model, policy, n_data);
	  end
	end
	printf("\tU:%f, F:%f, V:%f\n", PU(k, n_policies), PF(k, n_policies), PV(k, n_policies));
	fflush(stdout);
  end
end
