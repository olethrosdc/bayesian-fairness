...
lambda = repmat([0 0.25 0.5 0.75 1], 1, 4);
period = [repmat([100], 1, 10) repmat([1], 1, 10)];
alpha = repmat([1.0, 0.1], 1, 10);
n_iter = 10000 * ones(1,20);
options.model = test_model;
options.data = train_data;
options.utility = decision_problem.util;
options.n_samples = 16;
options.base_name="compas-seq-";
%%%for k=1:5
%%%  results{k} = SequentialSamplingDirichletFromData(options.model, options.data, %%%options.utility, lambda(k), n_iter(k), period(k), alpha(k))
%%%end
results = pararrayfun(8, @SequentialSamplingDirichletFromDataPar, options, lambda, n_iter, period, alpha);

%save ("compas-seq-iter-10000-period-1-samples-16-alpha-1.0.dat", "results", "options", "test_model");
